<script setup lang="ts">
import { getMainNews } from './services'
import { onMounted, ref } from 'vue'

// Swiper imports
import 'swiper/css'
import 'swiper/css/free-mode'
import 'swiper/css/pagination'


import { Autoplay, FreeMode, Pagination } from 'swiper/modules'
import { Swiper, SwiperSlide } from 'swiper/vue'

const lastNews = ref<any>(null)

onMounted(async () => {
   try {
      lastNews.value = await getMainNews()
      console.log(lastNews.value)
   } catch (error) {
      console.error(error)
   }
})

// Expose Swiper modules
const modules = [FreeMode, Pagination , Autoplay]
</script>


<template>
   <div class="grid grid-cols-2 gap-3 ">
      <div class="flex flex-col">
         <div>
            <img src="@/assets/icons/logo2.png" alt="">
         </div>

         <div>
            <swiper :slidesPerView="2" :loop="true" :autoplay="true" :spaceBetween="30" :freeMode="true" :pagination="{
               clickable: true,
            }" :modules="modules" class="mySwiper">
               <swiper-slide><img
                     src="data:image/png;base64,"
                     alt=""></swiper-slide>
               <swiper-slide>Slide 2</swiper-slide><swiper-slide>Slide 3</swiper-slide>
               <swiper-slide>Slide 4</swiper-slide><swiper-slide>Slide 5</swiper-slide>
               <swiper-slide>Slide 6</swiper-slide><swiper-slide>Slide 7</swiper-slide>
               <swiper-slide>Slide 8</swiper-slide><swiper-slide>Slide 9</swiper-slide>
            </swiper>

         </div>



      </div>


      <div class="flex flex-col">

         <div class="container mx-auto px-4 py-8">


            <div class="">
               <h1 class="text-xl text-slate-900 py-2 border-b mb-2 font-bold border-slate-900">Yangiliklar</h1>
               <div v-for="(item, index) in lastNews" :key="index"
                  class="flex flex-col md:flex-row overflow-hidden bg-white border-b border-slate-300 rounded-sm ">
                  <!-- Image section -->
                  <div class="md:w-1/5 h-30 md:h-auto">
                     <img :src="`https://fargona24.uz/storage/${item.photo}`" :alt="item.title"
                        class="w-full h-full object-cover">
                  </div>

                  <div class="md:w-2/3 p-1 md:p-2 ">
                     <h2 class="text-sm font-bold mb-2 text-slate-950 hover:text-[#173044] ">{{ item.name }}</h2>
                  </div>
               </div>
            </div>
         </div>
      </div>
   </div>
</template>

<style scoped>

</style>
